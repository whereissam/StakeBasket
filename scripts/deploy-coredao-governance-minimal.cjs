...
async function main() {
  ...
  const basketToken = await StakeBasketTokenFactory.deploy(
    "StakeBasket Token",
    "BASKET",
    deployer.address
  );
  ...
  // Set deployer as the StakeBasket contract so we can mint tokens for testing
  await basketToken.setStakeBasketContract(deployer.address);
  console.log("✅ Set deployer as StakeBasket contract for testing");

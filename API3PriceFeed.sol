...
contract API3PriceFeed is Ownable {
    ...
    // Price staleness threshold (24 hours for production)
    ...
    function getPrice(string memory asset) external view returns (uint256 price) {
        ...
        if (data.useAPI3 && data.dAPIAddress != address(0)) {
            ...
            try IApi3ReaderProxy(data.dAPIAddress).read() returns (int224 value, uint32 timestamp) {
                ...
                require(block.timestamp - timestamp <= MAX_PRICE_AGE, "API3 price stale");
                ...
                require(block.timestamp - data.lastUpdated <= MAX_PRICE_AGE, "Fallback price stale");
                return data.price;
            }
        } else {
            // Use manual price
            require(block.timestamp - data.lastUpdated <= MAX_PRICE_AGE, "Price data stale");
            return data.price;
        }
    }
    ...
    function isPriceFresh(string memory asset) external view returns (bool) {
        ...
        if (data.useAPI3 && data.dAPIAddress != address(0)) {
            ...
            try IApi3ReaderProxy(data.dAPIAddress).read() returns (int224 value, uint32 timestamp) {
                return (value > 0 && block.timestamp - timestamp <= MAX_PRICE_AGE);
            } catch {
